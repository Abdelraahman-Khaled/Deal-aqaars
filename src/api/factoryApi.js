...
const FactoryAPI = {
  ...
  getFactoryByUserId: async (userId) => {
    try {
      const response = await axiosInstance.get(`/factory/user/${userId}`);
      return response.data;
    } catch (error) {
      console.error(
        `Error fetching factory for user ${userId}:`,
        error.response || error.message
      );
      handleError(error);
      throw error;
  ...
  createFactory: async (formData) => {
    try {
      ...
      const response = await axiosInstance.post("/factory", formData, {
        headers: {
  ...
  deleteFactory: async (id) => {
    try {
      const response = await axiosInstance.delete(`/factory/${id}`);
      toast.success(getToastMessages().deleteSuccess[getCurrentLanguage()]);
      return response.data;
    } catch (error) {
      console.error(
        `Error deleting factory with ID ${id}:`,
        error.response || error.message
      );
      handleError(error);
      throw error;
  ...
  // Search properties
  searchFactory: async (searchQuery, filters = {}) => {
    try {
      const queryParams = new URLSearchParams();
